  ...
  return (
    <motion.li
      ...
      initial={
        reduce
          ? { opacity: 0 }
          : { opacity: 0, y: -12, scale: 0.96, filter: "blur(10px)" }
      }
      animate={
        reduce
          ? { opacity: 1 }
    ...
    >
      <div
        className={cn(
          "relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/95 p-3 shadow-2xl backdrop-blur-xl",
          classNames?.surface,
        )}
      >
        {renderToast ? (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className={cn("min-w-0 flex-1", classNames?.content)}>
              <AnimatePresence mode="popLayout" initial={false}>
                ...
                >
                  <p
                    className={cn(
                      "truncate text-sm font-medium leading-5 text-zinc-100",
                      classNames?.title,
                    )}
                  >
                    {toast.title}
                  ...
                  {toast.description ? (
                    <p
                      className={cn(
                        "mt-0.5 line-clamp-2 text-xs leading-4 text-zinc-400",
              ...
              {toast.action ? (
                <button
                  ...
                  className={cn(
                    "mt-2 inline-flex h-7 items-center rounded-full bg-white/10 px-3 text-xs font-medium text-white transition-colors hover:bg-white/20",
                    classNames?.action,
                  )}
                >
                  {toast.action.label}
            ...
            {canDismiss ? (
              <button
                ...
                className={cn(
                  "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-zinc-500 transition-colors hover:bg-white/10 hover:text-white",
                  classNames?.close,
                )}
              >
